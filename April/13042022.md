...
<ol>
  ...
  - simple component for displaying a button with a mandatory title prop and detecting a press. It comes with its own styling and only the color can be modified. It does not accept the style prop, so if we require more customisation we should use the Touchable opacity instead.
